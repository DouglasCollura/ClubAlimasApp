<template>
			...
			<div class="section-user-top">
				...
				<div class="section-user-top-header">
					<div style="position: relative;"> 
						<button @click="AbrirNotificaciones" class="section-user-top-header-bell"><i class="icon-bell"></i></button> 
						<div v-if="usuario.new_notificacion" class="bubble-bell"></div>
					</div>
					
					<div class="card-img" style="width:40px;height:40px" @click="this.$router.push('/ProfileUser')">
				...
				<div>
					...
					<div class="grid gc-2x-g" style="grid-gap: 10px;">
						...
						<div class="align-self-center">
							<h1 class="text-white text-right" style="font-size: 65px; line-height: 65px;"> {{usuario.puntos}} </h1>
						</div>
			...
			<div class="section-user-body">
				...
				<div class="card card-user-info">
					...
					<div class="grid align-content-start" style="grid-gap: 10px;">
						...
						<p>¡Te faltan {{puntos_faltantes}} puntos para acceder a mayores beneficios!</p>
					</div>
				...
				<div class="card card-user-info">
					...
					<div>
						<div class="grid justify-items-center">
							<div class="chart" data-percent="100">
								<span class="percent"> {{usuario.pts_canjeados}} </span>
							</div>
						</div> 
					</div>
					...
					<div class="grid align-content-start" style="grid-gap: 10px;">
						...
						<p>Has canjeado hasta el momento {{usuario.pts_canjeados}} puntos.</p>
					</div>

				</div>

				<div class="user-group-button">
					<button class="user-group-button-select" @click="cambiarFaseBeneficio(0,$event)">Beneficios disponibles</button>
					<button @click="cambiarFaseBeneficio(1,$event)">Lo que podrías obtener</button>
				</div>
				...
				<div v-show="fase_beneficio == 0" class="grid" style="grid-gap: 10px;">
					...
					<div class="card card-beneficio-disponible" @click="VerCupon(cupon)"  v-for="cupon in cupones" :key="cupon.id">
					...
					<div class="content-cupon-prox">
						...
						<div class="card card-prox" v-for="cupon in proximos" :key="cupon">
							...
							<div class="card-prox-img-content" >
								<div class="card-prox-correct"> <i class="icon-correct"></i> </div>
								<div class="card-img" style="width:100px;height:100px;border-radius:70px"> 
									<img :src="host+cupon.url_img"  alt="">
								</div>
							</div>

							<h1 v-if="cupon.descuento != null" class="text-green align-self-end font-400">Descuento: {{cupon.descuento}}%</h1>
							<h1 v-else class="text-green align-self-end font-400">Precio: S/. {{cupon.precio}} </h1>
	...
	<Canje v-if="fase_cupones == 1" :cupon="cupon" :puntos="usuario.puntos" :documento="usuario.dni" :id_usuario="usuario.id" @Return="Return" @UpdatePuntos="UpdatePuntos" />
	...
	export default{
		...
		//===================CARGA=====================================================
			async CargarUsuario(){
				...
				if(this.usuario == null){
					...
					if(!this.usuario.rango){
						...
						this.porcentaje = (100-((this.puntos_faltantes * 100)/this.usuario.rango)).toFixed(0);
					}
			...
			async CargarCupones(){
				...
				this.cupones=null;
				await axios.post(process.env.VUE_APP_API+'GetBeneficioPorPunto',params).then(response=>{
			...
			async CargarProximos(){
				...
				this.proximos=null;
				await axios.post(process.env.VUE_APP_API+'GetBeneficioProximos',params).then(response=>{
		...
		//===================FUNCIONES=====================================================
			...
			async Refresh(){
				...
				await axios.post(process.env.VUE_APP_API+'RefreshUser',params).then(response=>{
					this.$swal.close();
					localStorage.clear();
		...
		//===============FUNCIONES DE CONTROL==============================================
			...
			async UpdatePuntos(puntos, pts_canjeados){
				...
				this.usuario.pts_canjeados =parseInt(this.usuario.pts_canjeados, 10) + parseInt( pts_canjeados, 10);
				await this.Refresh();
				await this.CargarCupones();
			},
...
.fade-right-enter-to,.fade-left-enter-to{
	opacity:1;
...
.fade-right-leave-from,.fade-left-leave-from{
	opacity:1;
	transform:translateX(0px);
}
.fade-right-leave-to{
